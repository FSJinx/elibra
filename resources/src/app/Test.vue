    ...
    <aside class="w-80 border-r border-slate-200 bg-white flex flex-col justify-between">
      <div>
        ...
        <div class="p-6 border-b border-slate-100 flex items-center gap-3">
          <div class="h-9 w-9 rounded-lg bg-indigo-600 flex items-center justify-center text-white shadow-md shadow-indigo-100">
            <Logo width="18" />
          </div>
          <div>
          ...
          <div v-for="page in pages" :key="page.name" class="space-y-1">
            ...
            <button class="flex items-center justify-between w-full px-4 py-3 rounded-xl text-sm font-semibold text-slate-600 hover:text-slate-900 hover:bg-slate-50 transition-all duration-200 group" @click="togglePage(page.name)">
              <div class="flex items-center gap-3">
                <component :is="page.icon" class="h-4.5 w-4.5 text-slate-400 group-hover:text-indigo-600 transition-colors" />
                <span>{{ page.name }}</span>
              </div>

              <ChevronDown class="h-4 w-4 text-slate-400 transition-transform duration-300" :class="page.name && 'rotate-180 text-indigo-600'" />
            </button>
            ...
            <Transition enter-active-class="transition-[max-height,opacity] duration-300 ease-out overflow-hidden" leave-active-class="transition-[max-height,opacity] duration-250 ease-in overflow-hidden" enter-from-class="max-h-0 opacity-0" enter-to-class="max-h-40 opacity-100" leave-from-class="max-h-40 opacity-100" leave-to-class="max-h-0 opacity-0">
              <div v-show="currentPage === page.name" class="pl-9 space-y-1 overflow-hidden">
                <button v-for="child in page.children" :key="child.name" class="relative flex items-center w-full text-left px-4 py-2.5 rounded-lg text-sm font-medium transition-all duration-200" :class="[currentPage === page.name && currentComponent === child.component ? 'bg-indigo-50/70 text-indigo-600 font-semibold shadow-sm' : 'text-slate-500 hover:text-slate-800 hover:bg-slate-50']" @click="open(page.name, child.name)">
                  <!-- Active Dot Indicator -->
                  <span v-if="currentPage === page.name && currentComponent === child.component" class="absolute left-0 w-1 h-5 bg-indigo-600 rounded-r-full" />
                  {{ child.name }}
                </button>
              </div>
            </Transition>
      ...
      <div class="p-4 border-t border-slate-100 bg-slate-50/50">
        <div class="flex items-center gap-3 px-2 py-1.5">
          <div class="h-9 w-9 rounded-full bg-primary-light/50 border-2 border-white shadow-sm flex items-center justify-center font-bold text-indigo-600 text-sm">EL</div>
    ...
    <main class="flex-1 overflow-auto bg-slate-50 flex flex-col">
      ...
      <header class="h-16 border-b border-slate-200 bg-white px-8 flex items-center justify-between shadow-sm shrink-0">
        <div class="flex items-center gap-2 text-sm text-slate-500">
          ...
          <span class="text-slate-300">|</span>
          <span class="text-indigo-600 font-bold bg-indigo-50 px-2.5 py-1 rounded-md text-xs">{{ page.name }}</span>
        </div>
      </header>
      ...
      <div class="flex-1 p-8 max-w-4xl w-full mx-auto">
        <Transition name="fade-component" mode="out-in">
          <div class="bg-white border border-slate-200 rounded-2xl p-8 shadow-sm min-h-100 flex flex-col justify-center item-center divide-y divide-slate-200">
